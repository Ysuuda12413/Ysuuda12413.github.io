...
function Nolagz() {
	var Mode = value.createList("Mode", ["Packet", "Old", "New"] , "Packet");
//	var Regen = value.createBoolean("RegenOnly", true);
	var Speed = value.createInteger("Speed", 5, 10, 1000, 100);
...
var dely = 0;
    this.onUpdate = function () {
		...
		if(dely >= Delay.get()) {
	if(((OnGround.get() && mc.thePlayer.onGround) || OnGround.get() == true)) {
		switch(Mode.get()) {
		case "Packet":
